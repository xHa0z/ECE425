...
int ocp_map[4][4] = {{0,99,99,0},{0,0,0,0},{0,99,99,0},{0,99,0,0}};
...
//test goal_pos	
  ...
  */
  ...
  dirc_map[x_pos][y_pos] = 0;
  if(avi_map[x_pos][y_pos] != 99){
    for(int i=x_pos;i<4;i++){
      for(int j=y_pos;j<4;j++){
        /*
       if(dirc_map[i][j] != 99){ 
        if(((i+1)<=3) && (dirc_map[i+1][j] >= 50 ) && (dirc_map[i+1][j] != 99)){
            dirc_map[i+1][j] = dirc_map[i][j] + 1;
            printf("the current status is: %d, %d, %d\n",i,j,dirc_map[i+1][j]);
        }else {
            dirc_map[i+1][j] = dirc_map[i+1][j];
            printf("the current status is: %d, %d, %d\n",i,j,dirc_map[i+1][j]);
        }
         if(((i-1)>=0) && (dirc_map[i-1][j] >= 50) && (dirc_map[i-1][j] != 99)){
            dirc_map[i-1][j] = dirc_map[i][j] + 1;
            printf("the current status is: %d, %d, %d\n",i,j,dirc_map[i-1][j]);
        }else {
            dirc_map[i-1][j] = dirc_map[i-1][j];
         ...
         if(((j+1)<=3) && (dirc_map[i][j+1] >= 50) && (dirc_map[i][j+1] != 99)){
         ...
         if(((i-1)>=0) && (dirc_map[i][j-1] >= 50) && (dirc_map[i][j-1] != 99)){
            dirc_map[i][j-1] = dirc_map[i][j] + 1;
            printf("the current status is: %d, %d, %d\n",i,j,dirc_map[i][j-1]);
        }else {
            dirc_map[i][j-1] = dirc_map[i][j-1];
    ...
    if((j+1 < 4)&&(avi_map[i][j+1] == 50) ){
      if((avi_map[i][j] != 99) && (dirc_map[i][j] < 50)){
      dirc_map[i][j+1] = dirc_map[i][j]+1;
      }else {
        dirc_map[i][j+1] = dirc_map[i][j+1];
      }
